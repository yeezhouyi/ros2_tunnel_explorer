#!/usr/bin/env bash
# Stage 4B.4 smoke test — 3 runs to verify Type B alternating-pair detection
set -euo pipefail
...
OUT_ROOT="${HOME}/stage4b4_smoke/alternating_pair"
...
for run_id in 01 02 03; do
  ...
  if [ -f "$f" ]; then
    ...
    revisit=$(grep "Revisit rate" "$f" | sed 's/.*| \(.*\) |/\1/')
    ...
    echo "  run_${run_id}: ${status} goals=${goals} revisit=${revisit} osc=${osc_events} escape=${escape_events} typeA=${type_a} typeB=${type_b}"
  fi
done

echo ""
echo "--- [EntranceOscillation] lines ---"
...
grep -rn "\[OscillationEscape\]" "${OUT_ROOT}/"*/attempt_01/frontier_explorer.log 2>/dev/null | head -20 || echo "  (none found)"
