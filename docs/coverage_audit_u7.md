...
## 车道碎片化发现(2026-09-07,sandbox 全链诊断)

sandbox(tb3_sandbox)家具把 6 行车道切成 **33 段**,相邻段水平错位
10-23 格 → 垂直对齐的 U 型帽(|dx|≤1)几乎不触发(A* 兜底),而 A*
连接器在"下一条 lane 起点位于当前端点后方"时产生**参考 180° 反向**,
MPC 在该处 e_psi≈π → 线性化失效 → fallback 冻结(实测:7.3m 后
v=0/cmd=0 持续 660s)。

## 机动生成规格(修复方向,下一单元)

线性 MPC 的 Frenet 模型在 e_psi≈±π 处不适定——**这不是可修的 bug,
是模型边界**。修复 = 参考路径生成显式旋转机动:

1. lane 端点处插入"原地旋转"参考段(位置不变,参考 yaw 从 θ 转到
   θ±π,以 ω_max 限速);MPC 需增加旋转跟随模式(位置保持 + 航向
   跟踪),或由 arbiter 在旋转段切换到独立旋转控制器;
2. 或 planner 输出半圆帽且要求 lane 间距 = 2×r_cap(间距改为可调,
   cap 半径取 max(spacing/2, v/ω_max)——当前间距 0.30m 与
   r_min=0.15m 恰好匹配,但家具碎片化使对齐假设不成立);
3. 两者都需全链重跑验证 coverage_task 上升。

诚实结论:**全链审计的价值正是暴露这一模型边界**——离线基准
(单调轨迹)全绿与全链(折叠路径)stall 的落差,由"参考路径
非单调 + Frenet 线性化边界"共同构成,修复属于机动生成层而非调参。
